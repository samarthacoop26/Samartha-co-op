...
export function ProductOverviewCard({ category }: ProductOverviewCardProps) {
  ...
  return (
    <div className="group flex flex-col bg-white rounded-xl border border-gray-200 hover:border-[#FF6B00] shadow-xs hover:shadow-lg transition-all duration-300 overflow-hidden">
      ...
      <div className="relative w-full aspect-[16/10] bg-gray-100 overflow-hidden">
        ...
        {/* Product Count Pill */}
        <div className="absolute top-3.5 right-3.5 z-10 flex items-center gap-1.5 bg-white/95 backdrop-blur-md px-2.5 py-1 rounded-full text-[11px] font-semibold text-gray-800 shadow-xs">
      ...
      <div className="flex flex-col flex-grow p-5 sm:p-6 justify-between bg-white">
          ...
          <div className="pt-3 border-t border-gray-100 mb-5">
            ...
            <div className="flex flex-wrap gap-1.5">
              {category.products.slice(0, 5).map((prod) => (
                <span
                  key={prod.id}
                  className="text-xs bg-gray-50 text-gray-700 px-2.5 py-1 rounded-md font-medium border border-gray-200"
                >
                  {prod.name}
                </span>
              ))}
              {category.products.length > 5 && (
                <span className="text-xs bg-orange-50 text-[#FF6B00] px-2 py-1 rounded-md font-semibold border border-orange-100">
        ...
        <div className="pt-4 border-t border-gray-100">
          <Link
            ...
            className="w-full inline-flex items-center justify-center gap-2 bg-[#0A1628] hover:bg-[#FF6B00] text-white font-semibold text-xs uppercase tracking-wider py-3 px-4 rounded-lg transition-colors duration-200 text-center"
          >
